...
col2.metric(label="Active Members", value=len(unique_mbr), value=len(unique_mbr)-st.session_state['save_mbr_num'])
col3.metric(label="Active Endpoints", value=len(unique_endpoints), len(unique_endpoints)-st.session_state['save_end_num'])
col4.metric(label="All Requests", value=len(res.items),len(res.items)-st.session_state['save_req_num'] )
